...
class PedidoDeliveryController extends Controller
{
    ...
    {
        $this->normalizarPayloadsJson($request, ['detalles', 'pago']);
        ...
        $validated = $request->validate([
            ...
            'foto_frontis' => ['nullable', 'image', 'max:5120'],
        ...
        $fotoFrontisUrl = $this->guardarFotoFrontis($request) ?? ($validated['foto_frontis_url'] ?? null);
    ...
    public function update(Request $request, Pedido $pedido)
    {
        ...
        $this->normalizarPayloadsJson($request, ['pago']);
        ...
        $fotoFrontisUrl = $this->guardarFotoFrontis($request) ?? ($validated['foto_frontis_url'] ?? $pedido->foto_frontis_url);
    ...
    /**
     * Guardar foto del frontis en carpeta public/assets/delivery.
     */
    private function guardarFotoFrontis(Request $request): ?string
    {
        if (!$request->hasFile('foto_frontis')) {
            return null;
        }

        $archivo = $request->file('foto_frontis');
        if (!$archivo->isValid()) {
            return null;
        }

        $carpeta = public_path('assets/delivery');

        if (!File::exists($carpeta)) {
            File::makeDirectory($carpeta, 0755, true);
        }

        $nombre = uniqid('delivery_', true) . '.' . $archivo->getClientOriginalExtension();
        $archivo->move($carpeta, $nombre);

        return "assets/delivery/{$nombre}";
    }

    /**
     * Convertir payloads JSON enviados como string a array.
     *
     * @param array<int, string> $keys
     */
    private function normalizarPayloadsJson(Request $request, array $keys): void
    {
        $data = [];
