...
class PedidoDeliveryController extends Controller
{
    ...
    public function subirEvidenciaFrontis(Request $request)
    {
        $validated = $request->validate([
            'pedido_id' => ['required', 'integer', 'exists:pedidos,pedido_id'],
            'foto_frontis' => ['required', 'file', 'max:5120'],
        ]);

        $archivo = $validated['foto_frontis'];
        $pedido = Pedido::query()->findOrFail((int) $validated['pedido_id']);
        ...
        $rutaPublica = public_path('assets/images/img-frontis');
        ...
        $rutaFront = base_path('../front-pollo-fresco/src/assets/images/img-frontis');
        if (File::isDirectory(dirname($rutaFront)) || File::isDirectory(base_path('../front-pollo-fresco/src/assets/images'))) {
            try {
        ...
        $fotoFrontisUrl = asset('assets/images/img-frontis/' . $nombreArchivo);
        $pedido->foto_frontis_url = $fotoFrontisUrl;
        $pedido->save();
