...
class VentaController extends Controller
{
    ...
    {
        if (!$this->tablasDeVentaDisponibles()) {
            return response()->json([
                'message' => 'Las tablas de ventas aún no existen. Ejecuta las migraciones pendientes.',
            ], 409);
        }
    ...
    public function store(Request $request)
    {
        if (!$this->tablasDeVentaDisponibles()) {
            return response()->json([
                'message' => 'No se puede guardar la venta porque faltan tablas en la base de datos. Ejecuta migraciones.',
            ], 409);
        }
    ...
    public function pdf(Request $request, int $ventaId)
    {
        if (!$this->tablasDeVentaDisponibles()) {
            return response()->json([
                'message' => 'No se puede generar el PDF porque faltan tablas en la base de datos.',
            ], 409);
        }
    ...
    private function tablasDeVentaDisponibles(): bool
    {
        return Schema::hasTable('comprobantes_venta') && Schema::hasTable('comprobantes_venta_detalle');
    }
}
