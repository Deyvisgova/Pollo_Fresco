...
export class PrivadoPedidos implements OnInit {
  ...
  actualizarBusquedaProductos(): void {
    ...
    this.productosFiltrados = this.productosDisponibles.filter((producto) => producto.toLowerCase().includes(termino));
  }
  ...
  seleccionarProductoDesdeBuscador(): void {
    ...
    const existe = this.productosDisponibles.some((producto) => producto.toLowerCase() === nombre.toLowerCase());
    if (existe) {
      this.agregarLineaDetalle(nombre);
  ...
  private cargarProductos(search = ''): void {
    ...
    this.http.get<ProductoApi[]>('/api/otros-productos/productos', { headers: this.obtenerHeaders(), params }).subscribe({
      next: (productos) => {
        this.productosDisponibles = productos.map((p) => p.nombre);
      },
      error: () => {
