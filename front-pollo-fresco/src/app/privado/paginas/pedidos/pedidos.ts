...
export class PrivadoPedidos implements OnInit {
  ...
  seleccionarProductoDesdeBuscador(): void {
    ...
    const existe = this.productosDisponibles.some((producto) => producto.nombre.toLowerCase() === nombre.toLowerCase());
    if (!existe) {
      return;
    }

    const indiceFilaObjetivo = this.detalles.findIndex((item) => !item.descripcion.trim());
    const indice = indiceFilaObjetivo >= 0 ? indiceFilaObjetivo : 0;
    this.detalles[indice].descripcion = nombre;
